...
save_to_rmd_pdf <- function(laliste_rapport,dossier,filename,output_format="word_document") {
  ...
  for (j in seq_along(laliste_rapport)) {
    chemin_rapport %>%
      add_h(names(laliste_rapport)[j],3) %>%
      # add_graph(laliste_rapport[[j]])
      add_image(laliste_rapport[[j]])# ici savoir si on envoit le nom ou letitre
  }
browser()
  rmarkdown::render(chemin_rapport,
                    # output_file = filename,
